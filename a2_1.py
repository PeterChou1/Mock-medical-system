...
# define the normal nucleotides
nucleo_sym = 'ATCG'
# define the symbols used to symbolize left maternal or right maternal
mat_sym = ['LM','RM']
...
male_symbol = 'M'
female_symbol = 'F'
# define length of a pair in a chromosome
pair_length = 2
...
class ChromosomePair():
    ...
    def get_by_pos(self, pos):
        ...
        '''    
        # try to return a pair given a position if not such position was found
        # raise PairNotFoundError
        try:
...
class ChromosomeSet():
    ...
    def set_sex_chromo(self, new_pos):
        ...
        REQ: 0 <= new_pos <= self._total_pair
    ...
    def set_by_pos(self, pair_num, pos, new_pair):
        ...
        '''
        # try to acess the chromosome at the pair number and the set the new 
        # pair of nucleotide if chromosome is not found get chromosome method
        # will Raise Chromosome not found error
    ...
    def get_by_pos(self, pair_num, pos):
        ...
        '''
        # try to access the chromosome at the pair number and the nucleotide
        # which exist at position number. 
        # if chromosome doesn't exist method will raise ChromosomeNotFoundError
        chromosome_i = self.get_chromosome(pair_num)
    ...
    def set_by_marker(self, marker, new_pair):
        ...
        object
        '''
        # use get marker method to acquire the index method will raise
        # marker not found index if marker is not found
        (pair_num, pos) = self._get_by_marker_index(marker)
        # use set by pos method to change to new pair
        # set by pos will raise InvalidInputError if pair is not length two
    ...
    def get_by_marker(self, marker):
        ...
        # throw MarkerNotFound Error if marker was not found
        (pair_num, pos) = self._get_by_marker_index(marker)
...
class Human(ChromosomeSet):
    ...
    def __init__(self, human_id):
        ...
        # if self passed is in the female subclass
        if (type(self) is Female):
            # set sex to be the symbol denoting that obj is female
            self._sex = female_symbol
        # otherwise if self is Male
        elif(type(self) is Male):
            # set sex to be the symbol denoting that obj is female
            self._sex = male_symbol
        else:
            # otherwise set it to be X for to be determined
            self._sex = 'X'
    ...
    def set_id(self, new_id):
        ...
        # check if new id is in our list
    ...
    def get_sex(self):
        ...
        Note: we do not allow setting sex as setting sex from male
        to female would foundamentally change the data structure of the object
        '''
        return self._sex 
    ...
    def test(self, query):
        ...
        mem_to_nucleo = dict()
        # define can compare with human chromosome which by default we assume 
        # to be true
        can_compare = True
        ...
        while (can_compare and pair_num <= human_chromosome):
            ...
            for pos in pos_set:
                # define the actual compare value to be empty string
                q_compare = ''
...
class Query(ChromosomeSet):
    ...
    def __init__(self):
        ...
        '''
        # initialize how many chromosomes a human has
        ChromosomeSet.__init__(self, human_chromosome, h_chromo_sex)
...
class Female(Human):
    ...
    def procreate(self, father, binder):
        ...
        '''
        # get babies sex if it does not have sex method will Raise
        # SexlessBabyError
        sex = binder.get_sex()
        ...
        # if it is M
        if (sex == male_symbol):
            # create child to be Male with Id of two parents appended
            child = Male(self.get_id() + father.get_id())     
        # if it is F
        elif (sex == female_symbol):
            # create child to be Female with Id of two parents appended
            child = Female(self.get_id() + father.get_id())
        ...
        for b_chromo_num in range(0, human_chromosome + 1):
            ...
            for b_pos in b_chromo_pair:
                ...
                try:
                    ...
                    # no error was raised
                    # get binder at current position
...
if(__name__ == "__main__"):
    my_male = Male('131')
    my_query = Query()
    my_query.set_by_pos(0, 0, 'AA')
    my_query.set_by_pos(0, 1, 'CC')
    my_query.set_by_pos(0, 2, '98')
    my_query.set_by_pos(0, 3, '87')
    my_query.set_by_pos(1, 3, '79')
    my_male.set_by_pos(0, 0, 'AA')
    my_male.set_by_pos(0, 1, 'CC')
    my_male.set_by_pos(0, 2, 'AT')
    my_male.set_by_pos(0, 3, 'TG')
    my_male.set_by_pos(1, 3, 'GA')
    print(my_male.test(my_query))
    my_dad = Male('129')
    my_mom = Female('134')
    binder = Binder()
    binder.set_sex('F')
    my_dad.set_by_pos(0, 0, 'AA')
    my_dad.set_by_pos(10,202103123,'AT')
    my_mom.set_by_pos(0,18888,'AA')
    my_mom.set_by_pos(0, 0, 'TG') 
    binder.set_by_pos(0, 0, 'RM')
    child = my_mom.procreate(my_dad, binder)
    print(child.get_by_pos(0, 0))
